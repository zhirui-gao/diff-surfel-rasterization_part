...
renderCUDA(
	...
	int W, int H,const int Semantic_Types,
	...
	float* __restrict__ dL_dsemantics)
{
    ...
    const int MAX_SEMANTIC_TYPES = 16;
	...
	__shared__ float collected_semantics[MAX_SEMANTIC_TYPES* BLOCK_SIZE];
	...
	float accum_rec[C] = { 0 };
	float dL_dpixel[C];
	float accum_rec_semantic[MAX_SEMANTIC_TYPES] = {0};
	float dL_dpixel_semantic[MAX_SEMANTIC_TYPES];
	...
	float last_semantic[MAX_SEMANTIC_TYPES] = { 0 };
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i=0; i< Semantic_Types; i++)
			    collected_semantics[i*BLOCK_SIZE + block.thread_rank()] = semantics[coll_id * Semantic_Types + i];
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < Semantic_Types; ch++)
			{
				const float sem = collected_semantics[ch * BLOCK_SIZE + j];
				accum_rec_semantic[ch] = last_alpha * last_semantic[ch] + (1.f - last_alpha) * accum_rec_semantic[ch];
				last_semantic[ch] = sem;
				const float dL_dchannel_semantic = dL_dpixel_semantic[ch];
// 				dL_dalpha += (sem - accum_rec_semantic[ch]) * dL_dchannel_semantic;  //ackward_geometry
				atomicAdd(&(dL_dsemantics[global_id * Semantic_Types + ch]), dchannel_dcolor * dL_dchannel_semantic);
