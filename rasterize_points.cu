	...
	const bool debug)
{
  ...
  const int Semantic_Types = semantics.size(1);
  ...
  torch::Tensor out_semantic = torch::full({Semantic_Types,H,W},0.0,float_opts);
  ...
  return std::make_tuple(rendered, out_color,out_semantic, out_others, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dsemantics = torch::zeros({P, Semantic_Types}, means3D.options());
